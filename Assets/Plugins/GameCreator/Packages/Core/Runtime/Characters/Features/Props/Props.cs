...
namespace GameCreator.Runtime.Characters
{
    ...
    public class Props
    {
        ...
        [NonSerialized] private Dictionary<EntityId, List<IProp>> m_Props;
        ...
        public Props()
        {
            this.m_Props = new Dictionary<EntityId, List<IProp>>();
        }
        ...
        public bool HasInstance(GameObject instance)
        {
            ...
            foreach (KeyValuePair<EntityId, List<IProp>> entry in this.m_Props)
            {
                foreach (IProp prop in entry.Value)
                {
                    if (prop.Instance == null) continue;
                    if (prop.Instance.GetEntityId() == instanceID) return true;
                }
            }
        ...
        public void RemovePrefab(GameObject prefab, EntityId instanceID)
        {
            if (prefab == null) return;
            EntityId prefabInstanceID = prefab.GetEntityId();
            ...
            for (int i = 0; i < props.Count; i++)
            {
                ...
                if (prop.Instance == null) continue;
                if (prop.Instance.GetEntityId() != instanceID) continue;
        ...
        /// <param name="instanceID"></param>
        /// <returns></returns>
        public GameObject DropPrefab(GameObject prefab, EntityId instanceID)
        {
            if (prefab == null) return null;
            EntityId prefabInstanceID = prefab.GetEntityId();
