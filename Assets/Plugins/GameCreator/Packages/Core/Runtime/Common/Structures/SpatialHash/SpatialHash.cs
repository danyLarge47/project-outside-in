...
namespace GameCreator.Runtime.Common
{
    public class SpatialHash
    {
        ...
        private readonly Dictionary<EntityId, ISpatialHash> m_Instances = 
            new Dictionary<EntityId, ISpatialHash>();

        private NativeHashSet<EntityId> m_DynamicInstances =
            new NativeHashSet<EntityId>(INITIAL_CAPACITY, Allocator.Persistent);

        private NativeHashMap<EntityId, Record> m_Records =
            new NativeHashMap<EntityId, Record>(INITIAL_CAPACITY, Allocator.Persistent);
        
        private NativeHashMap<HashKey, int> m_HashCensus =
            new NativeHashMap<HashKey, int>(INITIAL_CAPACITY, Allocator.Persistent);
        
        private NativeParallelMultiHashMap<HashKey, EntityId> m_HashData =
            new NativeParallelMultiHashMap<HashKey, EntityId>(INITIAL_CAPACITY, Allocator.Persistent);
        ...
        public void Insert(ISpatialHash item)
        {
            ...
            EntityId uniqueCode = item.UniqueCode;
        ...
        public void Find(Vector3 point, float radius, List<ISpatialHash> results, ISpatialHash except = null)
        {
            ...
            {
                foreach (KeyValuePair<EntityId, ISpatialHash> entry in this.m_Instances)
                {
            ...
            else
            {
                ...
                this.GetCandidates(MAX_DIMENSION, point, radius, except?.UniqueCode ?? EntityId.None);
            }
            ...
            {
                EntityId uniqueCode = candidate.UniqueCode;
        ...
        private void GetCandidates(int dimension, float3 point, float radius, EntityId exceptUniqueCode)
        {
            ...
            for (int x = cellA.x; x <= cellB.x; ++x)
            {
                for (int y = cellC.y; y <= cellA.y; ++y)
                {
                    for (int z = cellC.z; z <= cellD.z; ++z)
                    {
                        ...
                        {
                            foreach (EntityId uniqueCode in this.m_HashData.GetValuesForKey(hashKey))
                            {
        ...
        private void UpdateRecords()
        {
            ...
            foreach (EntityId uniqueCode in this.m_DynamicInstances)
            {
