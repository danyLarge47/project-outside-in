...
namespace GameCreator.Editor.Common
{
    public static class TypeUtils
    {
        ...
        private static void RequireAssembliesTypesSetup()
        {
            ...
            IReadOnlyList<Assembly> assemblies = UnityEngine
                .Assemblies
                .CurrentAssemblies
                .GetLoadedAssemblies();
        ...
        private static Type[] GetDerivedTypes(Type type)
        {
            ...
            if (!type.IsInterface && !type.IsAbstract) types.Add(type);

            foreach (Type assemblyType in TypeCache.GetTypesDerivedFrom(type))
            {
                if (assemblyType == type) continue;
